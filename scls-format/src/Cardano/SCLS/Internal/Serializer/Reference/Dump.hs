...
data ManifestInfo = ManifestInfo
  { namespaceInfo :: Map Text NamespaceInfo
  }
...
mkManifest ManifestInfo{..} = do
  let ns = Map.toList namespaceInfo
      totalEntries = sum (namespaceEntries . snd <$> ns)
      totalChunks = sum (namespaceChunks . snd <$> ns)
      rootHash = fromMaybe (BS.replicate 28 0) $ listToMaybe $ namespaceHash . snd <$> ns -- TODO: combine hashes properly
  pure
    Manifest
      { totalEntries
      , totalChunks
